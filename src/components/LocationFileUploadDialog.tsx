...
const ACCEPT = FILE_CONTENT_TYPES.join(',');

let nextLocalId = 0;
const makeLocalId = () => `f-${++nextLocalId}`;

function validateFile(file: File): string | null {
  if (!(FILE_CONTENT_TYPES as readonly string[]).includes(file.type)) {
    return 'Unsupported type — JPEG, PNG, WebP, or PDF only';
  }
...
export default function LocationFileUploadDialog({ isOpen, onClose, locationId }: Props) {
  ...
  return (
    <Dialog open={isOpen} onClose={isUploading ? () => undefined : onClose} size="2xl">
      ...
      <DialogDescription>
        Files upload straight to this location’s record — photos and PDFs up to 25 MB. Job photos
        don’t need uploading here; they arrive with the work order.
      </DialogDescription>
