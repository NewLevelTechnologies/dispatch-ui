...
describe('RoleDetailPage', () => {
  ...
  describe('Permission-based UI', () => {
    it('hides Edit button when user lacks EDIT_ROLES capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap !== 'EDIT_ROLES');
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockRole });
    ...
    it('hides Restore Defaults button when user lacks EDIT_ROLES capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap !== 'EDIT_ROLES');
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockRole });
    ...
    it('hides Clone button when user lacks CREATE_ROLES capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap !== 'CREATE_ROLES');
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockRole });
      ...
      expect(screen.queryByRole('button', { name: /clone role/i })).not.toBeInTheDocument();
    });

    it('hides Delete button when user lacks DELETE_ROLES capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap !== 'DELETE_ROLES');
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockRole });
    ...
    it('hides all action buttons when user has no permissions', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
    ...
    it('shows Edit button for non-protected role when user has EDIT_ROLES', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap === 'EDIT_ROLES');
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockRole });
    ...
    it('respects isProtected flag even when user has EDIT_ROLES', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      ...
      // Protected roles cannot be edited or deleted even with permissions
      expect(screen.queryByRole('button', { name: /^edit$/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /^delete$/i })).not.toBeInTheDocument();
      // But clone should still be available
      expect(screen.getByRole('button', { name: /clone role/i })).toBeInTheDocument();
    });
  });
});
