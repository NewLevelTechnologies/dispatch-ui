...
describe('UsersPage', () => {
  ...
  describe('Permission-based UI', () => {
    it('hides Add User button when user lacks INVITE_USERS capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap !== 'INVITE_USERS');
      vi.mocked(apiClient.get).mockImplementation((url) => {
        if (url === '/users') {
          return Promise.resolve({ data: mockUsers });
        }
    ...
    it('shows Add User button when user has INVITE_USERS capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap === 'INVITE_USERS');
      vi.mocked(apiClient.get).mockImplementation((url) => {
        if (url === '/users') {
    ...
    it('hides Edit and Enable/Disable actions when user lacks EDIT_USERS capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockImplementation((cap: string) => cap === 'DELETE_USERS');
      vi.mocked(apiClient.get).mockImplementation((url) => {
        if (url === '/users') {
          return Promise.resolve({ data: mockUsers });
        }
      ...
      // Edit and Disable should not be present, but Delete should be
      expect(screen.queryByRole('menuitem', { name: /edit/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('menuitem', { name: /disable/i })).not.toBeInTheDocument();
      expect(screen.getByRole('menuitem', { name: /delete/i })).toBeInTheDocument();
    });

    it('hides Delete action when user lacks DELETE_USERS capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      ...
      // Edit and Disable should be present, but Delete should not be
      expect(screen.getByRole('menuitem', { name: /edit/i })).toBeInTheDocument();
      expect(screen.getByRole('menuitem', { name: /disable/i })).toBeInTheDocument();
      expect(screen.queryByRole('menuitem', { name: /delete/i })).not.toBeInTheDocument();
    });

    it('hides entire action menu when user has no permissions', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      ...
      // Dropdown button should not exist in the actions column
      expect(screen.queryByRole('button', { name: /more options/i })).not.toBeInTheDocument();
    });

    it('hides Add your first user button when user lacks INVITE_USERS capability', async () => {
      const { useHasCapability } = await import('../hooks/useCurrentUser');
      vi.mocked(useHasCapability).mockReturnValue(false);
      vi.mocked(apiClient.get).mockResolvedValue({ data: [] });
      ...
      expect(screen.queryByRole('button', { name: /add your first user/i })).not.toBeInTheDocument();
    });
  });
});
